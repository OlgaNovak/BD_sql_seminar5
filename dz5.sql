...
-- Задача 1. Создайте представление, в которое попадет информация о пользователях (имя, фамилия, город и пол), которые не старше 20 лет.
CREATE OR REPLACE VIEW view_users AS
   SELECT CONCAT(firstname,' ', lastname) as persons, hometown, gender
   FROM users u
   JOIN profiles p ON p.user_id=u.id
   WHERE p.birthday>='2003-06-08';
...
SELECT 
   DENSE_RANK() OVER (ORDER BY COUNT(from_user_id) DESC) AS rank_mes, COUNT(from_user_id) AS Number_of_mess,
  CONCAT(firstname,' ', lastname) as Persons 
FROM messages m 
JOIN users u ON m.from_user_id=u.id
GROUP BY m.from_user_id;

-- Задача 3.
/*Выберите все сообщения, отсортируйте сообщения по возрастанию даты отправления (created_at) и найдите разницу
 ...
 SELECT
    ...
    body, 
    created_at,
    (created_at - LEAD(created_at) OVER w) AS diff_sec
FROM messages
WINDOW w AS (PARTITION BY TIMESTAMPDIFF(SECOND, created_at,created_at));
